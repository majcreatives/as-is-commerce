...
| 31.5 | Registration redesign — split-screen desktop layout (dark marketing hero + white centered form), First/Last name inputs, required phone + optional email, show/hide password toggle; social login and terms checkbox omitted (no OAuth infrastructure; 31.3 legal pages remain deferred) | first deployed on `stage31.5` (two-column card); redesigned as a split-screen and deployed on `stage31.5b`; verified & deployed on staging (2026-09-25) — see `VERIFY_31_5B_REGISTER_SPLIT.md` and `VERIFY_31_5_REGISTRATION_REDESIGN.md` |
| 31.4, 31.7, 31.2b | Homepage discovery, newsletter popup, conditions as data | not started; see `PLAN_31_UX_BIDCAP_SHARING.md` |
