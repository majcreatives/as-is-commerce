...
| F Triggers / secrets / reconciliation | **PASS** — triggers refuse `total_minor`/`amount_minor` writes (`45000` both); `secret_hits=0`; every `charge.success` Processed + resolved to a distinct attempt/purchase, 0 unresolved. Visuals confirmed: Exception Centre shows `fulfilment_blocked` (`XMQS6ADA4C`) & `payment_after_close` (`YAWA5A3AH3`); `/admin/payments` + `/admin/payment-events` render refs/status/timestamps with no card data |
