...
final class UpdateRuleset
{
     ...
     */
    public function handle(AuctionRuleset $ruleset, array $attributes, ?User $actor = null, ?BidModel $model = null): AuctionRuleset
    {
        ...
        return app(CauserResolver::class)->withCauser(
            $actor,
            fn (): AuctionRuleset => $this->persist($ruleset, $attributes, $actor, $model),
        );
    }
     ...
     * @param  array<string, mixed>  $attributes
     */
    private function persist(AuctionRuleset $ruleset, array $attributes, ?User $actor, ?BidModel $model): AuctionRuleset
    {
        return DB::transaction(function () use ($ruleset, $attributes, $actor, $model): AuctionRuleset {
            $ruleset->fill($attributes);

            // Moving a draft to another bidding model is a deliberate act made
            // by code that means it -- the admin form moving an older draft to
            // the cumulative model -- and never something a payload can do.
            if ($model !== null) {
